...
impl Cpu{
    ...
    fn execute_data(&mut self, op: &DataOp, dst: &Register, src: &RegOrImm){
        ...
        match op{
            ...
            DataOp::STR => {
                self.mem.set(self.regs.get(dst) as u32, MemEntry::num(src_val));
            },
            DataOp::MOV => {
    ...
    fn execute_flow(&mut self, op: &FlowOp, offset: i32){
        if op.should_take(self.regs.get(&Register::ZR)){
            if let FlowOp::CALL = op{
                let sp = self.regs.get(&Register::SP);
                // push ret address
                self.mem.set(sp as u32, MemEntry::num(self.regs.get(&Register::IR) + 1));
                // push caller BP
                self.mem.set(sp as u32 - 1, MemEntry::num(self.regs.get(&Register::BP)));
